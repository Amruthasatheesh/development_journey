...
different_sources={di.get("source")for di in leads}
print(different_sources)
#get the all_sources_list and count
all_sources_list=[di.get("source")for di in leads]
print(all_sources_list)
source_count={s:all_sources_list.count(s)for s in all_sources_list}
print(source_count)
#status_summary
...
"""
#conversion_rate
qualified_rate=total no.of.qualified_lead/total no.of.lead
"""
qualified_rate=status_count.get("Qualified")/sum(status_count.values())
print("qualified rate",qualified_rate)
# unqualified_conversion_rate
unqualified_rate=status_count.get("Unqualified")/sum(status_count.values())
print("unqualified rate",unqualified_rate)
# want to get the percentage ==>*100
unqualified_rate_percentage=status_count.get("Unqualified")/sum(status_count.values())
print("unqualified percentage",unqualified_rate*100)
#converted_rate
converted_rate=status_count.get("Converted")/sum(status_count.values())
print(f"converted rate:{converted_rate}")
#Google_ads_conversion_rate
"""
google_ads_conversion_rate=no.of.converted google ads/total no of google ad
"""
# sir version
google_ad_status=[di.get("status") for di in leads if di.get("source")=="Google Ads"]
print("google ad status",google_ad_status)
google_status_summary={g:google_ad_status.count(g)for g in google_ad_status}
print("google ad summary",google_status_summary)
google_ad_converted_rate=google_status_summary.get("Converted",0)/sum(google_status_summary.values())
print(google_ad_converted_rate)# here there is no need to take the converted rate bcos its already shown/clr in the summary that is 0
#Another way to find this:
google_ad_converted=[di for di in leads if di.get("source")=="Google Ads" and di.get("status")=="Converted"]
print(len(google_ad_converted))
#course_lead
course_list=[di.get("course")for di in leads]
print("course list",course_list)
course_count={c:course_list.count(c)for c in course_list}
